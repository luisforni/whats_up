...
@router.get("/{user_id}/details", response_model=ProfileResponse)
def get_profile_details(user_id: int, db: Session = Depends(get_db)):
    profile = db.query(Profile).filter(Profile.user_id == user_id).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Perfil no encontrado")
    return profile
...
# Actualizar perfil
@router.put("/{user_id}/details", response_model=ProfileResponse)
...
def check_profile(user_id: int, db: Session = Depends(get_db)):
    profile = db.query(Profile).filter(Profile.user_id == user_id).first()
